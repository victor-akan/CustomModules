...
#     return bool(len(read_df_from_db(f"select * from user_tables where table_name = upper('{table_name}')")))

# def read_df_from_db(q):
#     con = get_conn_object()
...
# def execute_query_without_commit(query, con):
#     cursor = con.cursor()
#     cursor.execute(query)
#     cursor.close()

# def insert_rows(df, table_nm, con, chunksize=1000):
#     end_of_df=True
#     lower_limit = 0
#     while end_of_df:
...
# def get_files(target_directory, get_full_path=True):
...
#         if not does_dir_exists(self.etl_default_staging_parent_folder):
...
#         if does_table_exists(dest_table_name):
...
#         loaded_ref_dates = list(read_df_from_db(f"select distinct({dest_reference_date_field_name}) from {dest_table_name}")[dest_reference_date_field_name])
...
#         con = get_conn_object()
#         execute_query_without_commit(create_query, con)
#         con.close()
...
#         if(does_dir_exists(staging_directory)):
...
#             df = read_df_from_db(sourcing_query)
...
#         execute_query_without_commit(delete_query, con)    
#         con.commit()
#         con.close()

#         files = get_files(staging_directory)
#         for f in files:
#             print(f)
#             df = pd.read_csv(f)
#             print(df.dtypes)
...
#                     execute_query_without_commit(delete_query, con)
#                     print("insertion")
#                     insert_rows(df, dest_table_name, con=con)
#                     con.commit()
#                     con.close()
#                 except:
#                     con.rollback()
...
#         sourcing_dates_list = []
#         if etl_period==EtlPeriod.DAILY:
#             sourcing_dates_list = [etl_run_from_date+timedelta(days=f) for f in range(etl_period_freq)]
#         elif etl_period==EtlPeriod.WEEKLY:
#             sourcing_dates_list = []
